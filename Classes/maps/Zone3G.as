package maps {
	...
	public class Zone3G extends MapLoader {
		private var visited:Boolean = false;
		public var enemy1:Enemy;
		private var eName:String="z3ge1";

		public function Zone3G() {
			super("Zone3G");
			if (SharedObjectManager.getData("z3gv")==null) {SharedObjectManager.setData("z3gv", false);}
			else {visited = SharedObjectManager.getData("z3gv");}
		...
		override protected function onKeypress(ke:KeyboardEvent):void {
			...
			if (ke.keyCode==SharedObjectManager.getData("actionKey")) {
				if (isFacing(7,8)) {
					MovieClip(parent).transitionTo(MovieClip(parent).zone3F, 7, 1);return;
				}
				if (isFacing(0,4)) {
					MovieClip(parent).transitionTo(MovieClip(parent).zone3H, 12, 4);
				}
			}
		}
	}
