package maps {
	...
	public class Zone3I extends MapLoader {
		private var visited:Boolean = false;
		public var enemy1:Enemy;
		private var eName:String="z3ie1";

		public function Zone3I() {
			super("Zone3I");
			if (SharedObjectManager.getData("z3iv")==null) {SharedObjectManager.setData("z3iv", false);}
			else {visited = SharedObjectManager.getData("z3iv");}
		...
		override protected function onKeypress(ke:KeyboardEvent):void {
			...
			if (ke.keyCode==SharedObjectManager.getData("actionKey")) {
				if (isFacing(13,4)) {
					MovieClip(parent).transitionTo(MovieClip(parent).zone3J, 1, 4);return;
				}
				if (isFacing(7,8)) {
					MovieClip(parent).transitionTo(MovieClip(parent).zone3H, 7, 1);
				}
			}
		}
	}
