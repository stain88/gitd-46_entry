package maps {
	...
	public class Zone3D extends MapLoader {
		private var visited:Boolean = false;
		public var enemy1:Enemy;
		private var eName:String="z3de1";

		public function Zone3D() {
			super("Zone3D");
			if (SharedObjectManager.getData("z3dv")==null) {SharedObjectManager.setData("z3dv", false);}
			else {visited = SharedObjectManager.getData("z3dv");}
		...
		override protected function onKeypress(ke:KeyboardEvent):void {
			...
			if (ke.keyCode==SharedObjectManager.getData("actionKey")) {
				if (isFacing(7,0)) {
					MovieClip(parent).transitionTo(MovieClip(parent).zone3B, 7, 7);return;
				}
				if (isFacing(13,4)) {
					MovieClip(parent).transitionTo(MovieClip(parent).zone3E, 1, 4);
				}
			}
		}
	}
