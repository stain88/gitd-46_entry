package maps {
	...
	public class Zone3B extends MapLoader {
		private var visited:Boolean = false;
		public var enemy1:Enemy;
		private var eName:String="z3be1";

		public function Zone3B() {
			super("Zone3B");
			if (SharedObjectManager.getData("z3bv")==null) {SharedObjectManager.setData("z3bv", false);}
			else {visited = SharedObjectManager.getData("z3bv");}
		...
		override protected function onKeypress(ke:KeyboardEvent):void {
			...
			if (ke.keyCode==SharedObjectManager.getData("actionKey")) {
				if (isFacing(0,4)) {
					MovieClip(parent).transitionTo(MovieClip(parent).zone3A, 12, 4);return;
				}
				if (isFacing(7,0)) {
					MovieClip(parent).transitionTo(MovieClip(parent).zone3C, 7, 7);return;
				}
				if (isFacing(7,8)) {
					MovieClip(parent).transitionTo(MovieClip(parent).zone3D, 7, 1);return;
				}
				if (isFacing(13,4)&&MovieClip(parent).player.getItem("blueKey")) {
					MovieClip(parent).transitionTo(MovieClip(parent).zone3H, 1, 4);
				}
			}
		}
	}
