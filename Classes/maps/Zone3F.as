package maps {
	...
	public class Zone3F extends MapLoader {
		private var visited:Boolean = false;
		public var enemy1:Enemy;
		private var eName:String="z3fe1";

		public function Zone3F() {
			super("Zone3F");
			if (SharedObjectManager.getData("z3fv")==null) {SharedObjectManager.setData("z3fv", false);}
			else {visited = SharedObjectManager.getData("z3fv");}
		...
		override protected function onKeypress(ke:KeyboardEvent):void {
			...
			if (ke.keyCode==SharedObjectManager.getData("actionKey")) {
				if (isFacing(0,4)) {
					MovieClip(parent).transitionTo(MovieClip(parent).zone3E, 12, 4);return;
				}
				if (isFacing(7,0)) {
					MovieClip(parent).transitionTo(MovieClip(parent).zone3G, 7, 7);
				}
			}
		}
	}
