package maps {
	...
	public class Zone4C extends MapLoader {
		private var visited:Boolean = false;
		public var enemy1:Enemy;
		private var eName:String="z4ce1";

		public function Zone4C() {
			super("Zone4C");
			if (SharedObjectManager.getData("z4cv")==null) {SharedObjectManager.setData("z4cv", false);}
			else {visited = SharedObjectManager.getData("z4cv");}
		...
		override protected function onKeypress(ke:KeyboardEvent):void {
			...
			if (ke.keyCode==SharedObjectManager.getData("actionKey")) {
				if (isFacing(7,0)) {
					MovieClip(parent).transitionTo(MovieClip(parent).zone4B, 7, 7);return;
				}
				if (isFacing(7,8)) {
					MovieClip(parent).transitionTo(MovieClip(parent).zone4D, 7, 1);return;
				}
				if (isFacing(13,4)) {
					MovieClip(parent).transitionTo(MovieClip(parent).zone4E, 1, 4);
				}
			}
		}
	}
