package maps {
	...
	public class Zone4E extends MapLoader {
		private var visited:Boolean = false;
		public var enemy1:Enemy;
		private var eName:String="z4ee1";

		public function Zone4E() {
			super("Zone4E");
			if (SharedObjectManager.getData("z4ev")==null) {SharedObjectManager.setData("z4ev", false);}
			else {visited = SharedObjectManager.getData("z4ev");}
		...
		override protected function onKeypress(ke:KeyboardEvent):void {
			...
			if (ke.keyCode==SharedObjectManager.getData("actionKey")) {
				if (isFacing(0,4)) {
					MovieClip(parent).transitionTo(MovieClip(parent).zone4C, 12, 4);return;
				}
				if (isFacing(7,8)) {
					MovieClip(parent).transitionTo(MovieClip(parent).zone4F, 7, 1);
				}
			}
		}
	}
