package maps {
	...
	public class Zone2B extends MapLoader {
		private var visited:Boolean = false;
		public var enemy1:Enemy;
		private var eName:String="z2be1";

		public function Zone2B() {
			super("Zone2B");
			if (SharedObjectManager.getData("z2bv")==null) {SharedObjectManager.setData("z2bv", false);}
			else {visited = SharedObjectManager.getData("z2bv");}
		...
		override protected function onKeypress(ke:KeyboardEvent):void {
			...
			if (ke.keyCode==SharedObjectManager.getData("actionKey")) {
				if (isFacing(13,4)) {
					MovieClip(parent).transitionTo(MovieClip(parent).zone2A, 1, 4);return;
				}
				if (isFacing(0,4)) {
					MovieClip(parent).transitionTo(MovieClip(parent).zone2C, 12, 4);
				}
			}
		}
	}
