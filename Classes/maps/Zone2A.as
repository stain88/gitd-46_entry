package maps {
	...
	public class Zone2A extends MapLoader {
		private var visited:Boolean = false;
		public var enemy1:Enemy;
		private var eName:String="z2ae1";

		public function Zone2A() {
			super("Zone2A");
			if (SharedObjectManager.getData("z2av")==null) {SharedObjectManager.setData("z2av", false);}
			else {visited = SharedObjectManager.getData("z2av");}
		...
		override protected function onKeypress(ke:KeyboardEvent):void {
			...
			if (ke.keyCode==SharedObjectManager.getData("actionKey")) {
				if (isFacing(7,8)) {
					MovieClip(parent).transitionTo(MovieClip(parent).zone1D, 7, 1);return;
				}
				if (isFacing(0,4)) {
					MovieClip(parent).transitionTo(MovieClip(parent).zone2B, 12, 4);return;
				}
				if (isFacing(7,0)) {
					MovieClip(parent).transitionTo(MovieClip(parent).zone2D, 7, 7);
				}
			}
		}
	}
