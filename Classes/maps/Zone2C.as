package maps {
	...
	public class Zone2C extends MapLoader {
		private var visited:Boolean = false;
		public var enemy1:Enemy;
		private var eName:String="z2ce1";

		public function Zone2C() {
			super("Zone2C");
			if (SharedObjectManager.getData("z2cv")==null) {SharedObjectManager.setData("z2cv", false);}
			else {visited = SharedObjectManager.getData("z2cv");}
		...
		override protected function onKeypress(ke:KeyboardEvent):void {
			...
			if (ke.keyCode==SharedObjectManager.getData("actionKey")) {
				if (isFacing(13,4)) {
					MovieClip(parent).transitionTo(MovieClip(parent).zone2B, 1, 4);return;
				}
				if (isFacing(7,0)) {
					MovieClip(parent).transitionTo(MovieClip(parent).zone2F, 7, 7);
				}
			}
		}
	}
