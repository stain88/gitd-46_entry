package maps {
	...
	public class Zone2F extends MapLoader {
		private var visited:Boolean = false;
		public var enemy1:Enemy;
		private var eName:String="z2fe1";

		public function Zone2F() {
			super("Zone2F");
			if (SharedObjectManager.getData("z2fv")==null) {SharedObjectManager.setData("z2fv", false);}
			else {visited = SharedObjectManager.getData("z2fv");}
		...
		override protected function onKeypress(ke:KeyboardEvent):void {
			...
			if (ke.keyCode==SharedObjectManager.getData("actionKey")) {
				if (isFacing(7,8)) {
					MovieClip(parent).transitionTo(MovieClip(parent).zone2C, 7, 1);return;
				}
				if (isFacing(7,0)) {
					MovieClip(parent).transitionTo(MovieClip(parent).zone2I, 7, 7);
				}
			}
		}
	}
