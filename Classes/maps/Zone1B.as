package maps {
	...
	public class Zone1B extends MapLoader {
		private var visited:Boolean = false;
		public var enemy1:Enemy;
		private var eName:String="z1be1";
		private var zName:String="z1b";

		public function Zone1B() {
			super("Zone1B");
		...
		override protected function addedHandler(e:Event):void {
			...
			if (SharedObjectManager.getData(eName)==null) {
				enemy1 = new Enemy(12, 4, 2000, 0.1);
				enemy1.rotation = 180;
		...
		override protected function onKeypress(ke:KeyboardEvent):void {
			...
			if (ke.keyCode==SharedObjectManager.getData("actionKey")) {
				if (isFacing(0,4)) {
					MovieClip(parent).transitionTo(MovieClip(parent).zone1A, 12, 4);return;
				}
				if (isFacing(13,4)) {
					MovieClip(parent).transitionTo(MovieClip(parent).zone1C, 1, 4);
				}
			}
		}
	}
