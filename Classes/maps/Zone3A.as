package maps {
	...
	public class Zone3A extends MapLoader {
		private var visited:Boolean = false;
		public var enemy1:Enemy;
		private var eName:String="z3ae1";

		public function Zone3A() {
			super("Zone3A");
			if (SharedObjectManager.getData("z3av")==null) {SharedObjectManager.setData("z3av", false);}
			else {visited = SharedObjectManager.getData("z3av");}
		...
		override protected function onKeypress(ke:KeyboardEvent):void {
			...
			if (ke.keyCode==SharedObjectManager.getData("actionKey")) {
				if (isFacing(0,4)) {
					MovieClip(parent).transitionTo(MovieClip(parent).zone2D, 12, 4);return;
				}
				if (isFacing(13,4)) {
					MovieClip(parent).transitionTo(MovieClip(parent).zone3B, 1, 4);
				}
			}
		}
	}
