package maps {
	...
	public class Zone2E extends MapLoader {
		private var visited:Boolean = false;
		public var enemy1:Enemy;
		private var eName:String="z2ee1";

		public function Zone2E() {
			super("Zone2E");
			if (SharedObjectManager.getData("z2ev")==null) {SharedObjectManager.setData("z2ev", false);}
			else {visited = SharedObjectManager.getData("z2ev");}
		...
		override protected function onKeypress(ke:KeyboardEvent):void {
			...
			if (ke.keyCode==SharedObjectManager.getData("actionKey")) {
				if (isFacing(13,4)) {
					MovieClip(parent).transitionTo(MovieClip(parent).zone2D, 1, 4);return;
				}
				if (isFacing(7,0)) {
					MovieClip(parent).transitionTo(MovieClip(parent).zone2H, 7, 7);
				}
			}
		}
	}
